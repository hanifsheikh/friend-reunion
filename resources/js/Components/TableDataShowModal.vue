<template>
  <div class="py-3 w-screen h-screen flex absolute left-0 top-0 flex justify-center items-center inset-0 z-50 outline-none focus:outline-none bg-no-repeat bg-center bg-cover" id="modal-id" style="backdrop-filter: blur(2px);">
    ...
    <div class="w-full max-w-3xl p-5 relative mx-auto my-auto rounded-xl shadow-lg  bg-white">
      <div class="flex flex-row justify-between p-1 bg-white rounded-tl-lg rounded-tr-lg"> <svg @click="closeModal" class="w-6 h-6 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="flex flex-wrap" v-if="data">
        ...
        <div class="w-full md:w-1/2 lg:w-1/2 px-3 mt-6">
          ...
          <p class="font-light text-gray-600 py-1"> Date of Birth : <span class="font-medium" v-if="data.data.date_of_birth">{{ data.data.date_of_birth | moment("Do MMM YYYY") }}</span></p>
